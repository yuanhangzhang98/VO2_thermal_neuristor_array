...
class Solver:
    def __init__(self, d, batch, N, V, R, noise_strength, Cth_factor, couple_factor, width_factor, T_base,
        ...
        self.name_string = f'{N}_{V:.1f}_{1000 * noise_strength:.4f}_{Cth_factor:.4f}_{couple_factor:.4f}_' \
                           f'{width_factor:.4f}'
    ...
    def dynamics(self, save_traj=False, plot_2D=False):
        ...
        for i in trange(1, self.n_repeat + 1):
            ...
            if self.d == 2 and plot_2D and i > self.n_repeat - plot_epochs:
                I_traj = I_traj[0].reshape(self.Nx, self.Ny, -1).cpu().numpy()
                ...
                for j in plot_idx:
                    fig, ax = plt.subplots()
                    ax.imshow(I_traj[:, :, j], cmap='Blues', norm=matplotlib.colors.Normalize(vmin=0, vmax=I_traj.max()))
                    ax.set_axis_off()
                    fig.savefig(f'graphs/2D_{self.name_string}_{i}_{j}.png', dpi=72, bbox_inches='tight')
                    plt.close()
            del I_traj
        ...
        for i in trange(self.batch):
            ...
            name_string_i = f'{self.N}_{V:.1f}_{1000 * self.noise_strength:.4f}_{Cth_factor:.4f}_' \
                            f'{self.couple_factor:.4f}_{self.width_factor:.4f}'
            fig, ax = plt.subplots()
            ax.imshow(I_traj_end[i], cmap='Blues')
            ax.set_xlabel('Time step')
            ax.set_ylabel('Neuron Index')
            fig.savefig(f'graphs/I-t_{name_string_i}.png', dpi=72, bbox_inches='tight')
            plt.close()
    ...
    def find_avalanches(self, binned_traj, save_binned_peaks=False):
        ...
        for i in trange(self.batch):
            ...
            ax.set_ylabel('Rescaled Neuron Index')
            fig.savefig(f'graphs/peaks_{name_string_i}_{self.len_x}_{self.len_y}.png', dpi=72, bbox_inches='tight')
            plt.close()
    ...
    def run(self, save_traj=False, save_binned_peaks=False, plot_2D=False):
        binned_peaks = self.dynamics(save_traj, plot_2D)
        IQR = self.find_avalanches(binned_peaks, save_binned_peaks)
        return IQR